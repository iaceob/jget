...
public class JobModel {
    ...
    private Boolean createJob(String id, String name, String suffix, Long size,
                          String path, String url, JobType type) {
        String sql = SqlKit.getSql("Job.createJob");
        return Db2.update(sql, id, name, suffix, size, path, url, type.getIndex())!=0;
    }
    ...
    public Boolean createJob(String id, String name, String suffix, Long size,
                             String path, String url, JobType type, String cli) {
        Boolean res = Db2.tx(()->{
            if (!JobModel.dao.createJob(id, name, suffix, size, path, url, type)) return false;
            if (!JobModel.dao.saveJobCli(id, cli)) return false;
            if (!JobModel.dao.saveJobStat(id, JobStat.EXTRACTING)) return false;
